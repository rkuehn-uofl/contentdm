<?php
	...
	foreach ($collectionList as $colLists => $colList) {
		...
		$objectString = file_get_contents($baseURL.$port.'/dmwebservices/index.php?q=dmGetCollectionFieldVocabulary'.$colList['alias'].'/people/0/0/json');
	...
	// Return Object Links
	echo '<ul id="people_browse">';
	
	foreach ($objectUnique as $key => $value) {
		
		echo '<li><a href="'.$baseURL.'/cdm/search/searchterm/'.$value.'/field/people!all/mode/exact!none/conn/and!and/order/nosort/">'.$value.'</a></li>';
